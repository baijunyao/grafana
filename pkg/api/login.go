...
func (hs *HTTPServer) LoginAPIPing(c *models.ReqContext) utils.Response {
...
func (hs *HTTPServer) LoginPost(c *models.ReqContext, cmd dtos.LoginCommand) utils.Response {
	authModule := ""
	var user *models.User
	var response *utils.NormalResponse
...
func (hs *HTTPServer) RedirectResponseWithError(ctx *models.ReqContext, err error, v ...interface{}) *utils.RedirectResponse {
	ctx.Logger.Error(err.Error(), v...)
	if err := hs.trySetEncryptedCookie(ctx, loginErrorCookieName, err.Error(), 60); err != nil {
		hs.log.Error("Failed to set encrypted cookie", "err", err)
	}
